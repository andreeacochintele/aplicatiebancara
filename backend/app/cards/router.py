...
@router.patch("/{card_id}/pin", response_model=CardPublic)
def update_card_pin(
    card_id: uuid.UUID,
    payload: CardPinUpdate,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
) -> CardPublic:
    card = CardService(db).update_pin(current_user.id, card_id, payload.pin)
    db.commit()
    return card


@router.post("/{card_id}/reveal", response_model=CardSensitiveDetails)
def reveal_card_details(
    ...
    return CardService(db).reveal_details(current_user.id, card_id, payload.pin)
