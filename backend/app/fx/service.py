...
get_quote() and get_market_rate() (the Wallets-page reference-rate display)
now price off the exact same rate: live ECB rates with a small markup,
falling back to the static _RATES_TO_RON table on any network failure. They
used to diverge — get_quote() priced off the static table unconditionally
while the display banner showed the live rate — which made the "Get quote"
result contradict the number the user had just been shown. get_rate() (the
static-only lookup) is kept for callers that intentionally want deterministic,
offline pricing unrelated to a user-facing quote: cashback FX conversion
(merchants), net-worth base-currency conversion (analytics), and one payments
flow — none of those show a live rate to the user first, so there's nothing
...
class FXService:
    ...
    def get_market_rate(self, source_currency: str, target_currency: str) -> Decimal:
        """Live reference rate with a small margin baked in. Used both for
        display (Wallets) and to price get_quote() — same rate, same margin,
        so what the user is shown is exactly what they get."""
        source_currency, target_currency = source_currency.upper(), target_currency.upper()
    ...
    def get_quote(self, user_id: uuid.UUID, data: FXQuoteRequest) -> FXQuote:
        ...
        rate = self.get_market_rate(source_currency, target_currency)
